...
class ShaderProgram {
  ...
  setShowObstacles(b: number) {
    this.use();
    if (this.unifShowObstacles !== -1)
    {
      gl.uniform1f(this.unifShowObstacles, b);
    }
  }
  ...
  setGenerationType(type: number) {
    this.use();
    if (this.unifGenType !== -1) {
      gl.uniform1f(this.unifGenType, type);
    }
  }
