...
const creep_spawn = 0.3;
const crawl_persist = 0.2;
const street_spacer = 150;
const street_random = 40;
const street_decay = 0.01;

function canMerge(p1: {x: number, y: number, a: number}, p2: {x: number, y: number, a: number}) {
    ...
    return dist([p3.x, p3.y], [p2.x, p2.y]) < 0.2*d;
}
...
class Road extends Drawable {
  ...
  constructor(spawn: {x: number, y: number}, ddir: number, headcount: number, id: any, type: any, terrain: Terrain) {
    ...
    for(let i = 0; i < headcount; i++) {
        ...
        this.heads[i].rbranch = Math.random() * street_random;
        this.heads[i].lbranch = Math.random() * street_random;
    }
  }
        ...
        for(let i = 0; i < div; i++) {
            ...
            if(!merged) {
                for(let p of [[this.nodes[lastNode].rbranch, Math.PI/2], [this.nodes[lastNode].lbranch, -Math.PI/2]]) {
                    if(p[0] > street_spacer) {
                        let bestA = null;
                        let maxPop = creep_spawn * 5;
                        ...
                        if(bestA != null) {
                            ...
                            if(p[1] > 0) newNode.rbranch = Math.random()*street_random*3;
                            else newNode.lbranch = Math.random()*street_random*3;
                        }
                    }
                }
            }
                        ...
                        if(p[0] > street_spacer*1.5) {
                            ...
                            if(otherNodes.length == 0) {
                                ...
                                if(bestA != null) {
                                    ...
                                    if(p[1] > 0) newNode.rbranch = Math.random()*street_random;
                                    else newNode.lbranch = Math.random()*street_random;
                                }
                            }
                        }
                    }
  ...
  addNode(pos: {x: number, y: number}, a: number) : {node: Node, merged: boolean} {
    ...
    if(this.type == 1) {
            ...
            for(let on of otherNodes) {
                if(on.data.type == 1) {
                    if(on.data.id != this.id || (Math.abs(this.nodecount - on.data.step) > 5*this.detail)) { // need to connect this road to a different road due to collision
                        ...
                        if(canMerge({x: pos.x, y: pos.y, a: a}, {x: on.data.x, y: on.data.y, a: on.data.a})) {
        ...
        let otherNodes = this.terrain.nodes.query(new Circle(pos.x, pos.y, 3));
        for(let on of otherNodes) {
        ...
        if(this.terrain.getPop(pos.x, pos.y) < 0.1) {
            toMerge = true;
        }
    }
...
class Sprawler extends Road {
    ...
    constructor(spawn: {x: number, y: number}, ddir: number, id: any, type: any, terrain: Terrain) {
        super(spawn, ddir, 0, id, type, terrain);
        this.center = spawn;
        for(let delta of deltas) {
            this.queue.push({p: {x: 0, y: 0}, d: delta});
        }
    }
    sprawl(): void {
        let new_queue: any = [];
        for(let head of this.queue) {
            let long = 3;
            let lastNode = head.id;
    ...
    gridToCoord(x: number, y: number) {
        //return {x: this.center.x + Math.cos(this.angle)}
    }
}
